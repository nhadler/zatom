...
class DrawElementsBase:
    ...
    def __init__(self, name, coordinates, element_properties=None, group_properties=None):
        ...
        if element:
            if len(value) != len(self._coordinates):
                raise AssertionError(
                    f"property '{name}' does not have the same length " "as the number of elements"
                )
            assert name not in self._grp_props, f"{name} is already set as a group property"
            self._el_props[name] = value
        else:
            assert name not in self._el_props, f"{name} is already set as an element property"
