...
import zatom.tools.ase_notebook.draw_elements as draw
...
def get_rotation_matrix(rotations, init_rotation=None):
    ...
    for i, a in [("xyz".index(s[-1]), radians(float(s[:-1]))) for s in rotations.split(",")]:
        s = sin(a)
        c = cos(a)
        if i == 0:
...
def get_cell_coordinates(cell, origin=(0.0, 0.0, 0.0), show_repeats=None, dash_pattern=None):
    ...
    for rep_a, rep_b, rep_c in product(
        *(range(1, reps_a + 1), range(1, reps_b + 1), range(1, reps_c + 1))
        ...
        rep_origin = (
            np.array(origin) + (rep_a - 1) * vec_a + (rep_b - 1) * vec_b + (rep_c - 1) * vec_c
        )
        if has_a:
    ...
    if dash_pattern:
        ...
        for start, end in lines:
            ...
            for n in range(ndashes - 1):
                dash_end = start + (end - start) * ((dash_fraction * n) + length_fraction)
...
def initialise_element_groups(
    ...
    if miller_planes is not None:
        ...
        for i, plane in enumerate(miller_planes):
            miller_points = get_miller_coordinates(atoms.cell, [plane[n] for n in "hkl"]).tolist()
    ...
    if show_bonds:
        ...
        if bond_array_name is not None:
            bonds = filter_bond_indices(bonds, atoms.get_array(bond_array_name).tolist())
        if bond_pairs_filter is not None:
            # ensure bi-directional
            allowed = set(
                [(a, b) for a, b in bond_pairs_filter] + [(b, a) for a, b in bond_pairs_filter]
            )
