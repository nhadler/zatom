...
def type_to_sphinx(typ, field_name):
    ...
    # TODO: Add a better implementation of type_to_sphinx
    if typ is None:
        field_type = "Any"
    ...
    elif hasattr(typ, "__name__") and getattr(typ, "__module__", "builtins") != "builtins":
...
def autodoc(attrs_class):
    ...
    The docstring will include the `attributes <attr.ib>`,
    ...
    * Contains a metadata dictionary with the keys
    ...
    if attrs_class.__doc__ and params_section:
        attrs_class.__doc__ = f"{fix_indent(attrs_class.__doc__).rstrip()}\n\n{params_section}"
    elif params_section:
