...
from zatom.tools.ase_notebook.attr_doc import autodoc
from zatom.tools.ase_notebook.color import Color
...
def in_range(minimum=None, maximum=None, inclusive_min=True):
    ...
    def _validate(self, attribute, value):
        ...
        if not isinstance(value, (int, float)):
            ...
            (inclusive_min and value < minimum) or (not inclusive_min and value <= minimum)
        ):
            raise_error = True
...
class ViewConfig:
    ...
    element_colors: str = attr.ib(
        ...
        metadata={"help": "Element to color mapping to use, if ``atom_color_by='element'``."},
    )
    ...
    atom_label_array: str = attr.ib(
        default="",
        validator=instance_of(str),
        metadata={"help": "The array name to use, if ``array`` chosen for ``atom_label_by``"},
    )
    ...
    atom_color_array: str = attr.ib(
        default="",
        validator=instance_of(str),
        metadata={"help": "The array name to use, if ``array`` chosen for ``atom_color_by``"},
    )
    atom_colormap: str = attr.ib(
        default="jet",
        validator=instance_of(str),
        metadata={"help": "The matplotlib colormap to use with ``atom_color_by`` values"},
    )
    ...
    atom_opacity: float = attr.ib(default=0.95, validator=in_range(0, 1), metadata={"help": ""})
    ...
    ghost_stroke_width: float = attr.ib(default=0.0, validator=in_range(0), metadata={"help": ""})
    ghost_lighten: float = attr.ib(
        default=0.0,
        validator=in_range(0),
        metadata={"help": "Lighten the ghost atom colors by this fraction."},
    )
    ghost_opacity: float = attr.ib(default=0.4, validator=in_range(0, 1), metadata={"help": ""})
    ...
    bond_color_by: str = attr.ib(
        ...
        metadata={"help": "How to color bond: 'atoms' (same as connecting atoms) or 'length'."},
    )
    bond_colormap: str = attr.ib(
        default="jet",
        validator=instance_of(str),
        metadata={"help": ("The matplotlib colormap to use with ``bond_color_by='length'``")},
    )
    bond_colormap_range: Union[list, tuple] = attr.ib(
        default=(None, None),
        validator=deep_iterable(optional(instance_of((int, float))), iterable_length(2)),
        metadata={
            "help": (
                "The matplotlib colormap normalisation to use with " "``bond_color_by='length'``"
            )
        },
    )
    ...
    miller_planes: Tuple[dict] = attr.ib(
        ...
        metadata={"help": "List of dictionaries, describing miller index planes to create."},
    )
    ...
    show_axes: bool = attr.ib(
        default=True,
        validator=instance_of(bool),
        metadata={"help": ("Show the 'world' axes, " "at a corner of the visualisation.")},
    )
    axes_uc: bool = attr.ib(
        default=False,
        validator=instance_of(bool),
        metadata={"help": ("Show the 'unit cell' axes (abc), instead of the 'world' axes (XYZ)")},
    )
    ...
    axes_line_color: str = attr.ib(default="black", validator=is_html_color, metadata={"help": ""})
    ...
    def _validate_uc_dash_pattern(self, attribute, value):
        ...
        if not isinstance(value, (list, tuple)) or len(value) != 2:
            raise TypeError(f"'{attribute.name}' must be of the form (line_length, gap_length).")
    ...
    def _bond_pairs_filter(self, attribute, value):
        ...
        for i, item in enumerate(value):
            try:
                ...
                raise TypeError(f"'{attribute.name}' item {i} must be a pair of strings.")
    ...
    def _validate_canvas_crop(self, attribute, value):
        ...
        if not isinstance(value, (list, tuple)) or len(value) != 4:
            raise TypeError(f"'{attribute.name}' must be of the form (left, right, top, bottom).")
