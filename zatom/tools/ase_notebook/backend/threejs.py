...
from zatom.tools.ase_notebook.draw_utils import triangle_normal
...
def generate_3js_render(
    ...
    Regarding initialisation performance, see:
    https://github.com/jupyter-widgets/pythreejs/issues/154
    """
    ...
    for el_hash, els in unique_atom_sets.items():
        ...
        if reuse_objects:
            atom_geometry = atom_geometries.setdefault(
                el.sradius,
                pjs.SphereBufferGeometry(radius=el.sradius, widthSegments=30, heightSegments=30),
            )
        else:
        ...
        if reuse_objects:
            atom_material = atom_materials.setdefault(
                ...
                pjs.MeshLambertMaterial(color=el.color, transparent=True, opacity=el.fill_opacity),
            )
        else:
    ...
    if len(element_groups["bond_lines"]) > 0:
        ...
        bond_line_geo = pjs.LineSegmentsGeometry(
            ...
            colors=[[Color(c).rgb for c in el.color] for el in element_groups["bond_lines"]],
        )
    ...
    if len(element_groups["miller_lines"]) > 0:
        ...
        miller_lines = pjs.LineSegments2(geometry=miller_line_geo, material=miller_line_mat)
    ...
    camera_dist = maxp[2] + sqrt(maxp[0] ** 2 + maxp[1] ** 2) / tan(radians(camera_fov / 2))
...
def add_labels(element_groups, key_elements, use_label_arrays):
    ...
    for el_hash, els in unique_label_sets.items():
        ...
        if use_label_arrays:
            ...
            label_array = [
                pjs.Sprite(material=text_material, position=e.position.tolist()) for e in els
            ]
...
def create_world_axes(
    ...
    ax_line_geo = pjs.LineSegmentsGeometry(
        positions=[[[0, 0, 0], length * r / np.linalg.norm(r)] for r in initial_rotation],
...
def make_basic_gui(container):
    ...
    control_box_elements = ipyw.Box(element_controls, layout=ipyw.Layout(flex_flow="column"))
    ...
    ipyw.jslink((background_opacity, "value"), (container.element_renderer, "clearOpacity"))
    ...
    control_box_background = ipyw.Box(background_controls, layout=ipyw.Layout(flex_flow="column"))
    ...
    info_box = ipyw.HTML(
        ...
        color="grey",
        layout=ipyw.Layout(max_height="10px", margin="0px 0px 0px 0px", align_self="flex-start"),
    )
    ...
    if axes and container.element_renderer.height > 200:
        ...
        grid[:, 0] = ipyw.Box(axes, layout=ipyw.Layout(align_self="flex-end", flex_flow="column"))
...
def create_arrow_texture(width=2**9, height=2**9, color="red", right=True):
    ...
    if right:  # facing right
        ...
        for y in range(int(width / 4), int(width / 2)):
            for x in range(int(0 + height * y / (width)), int(height - height * y / (width))):
                array[x, y, :] = color_rgba
    else:  # facing left
