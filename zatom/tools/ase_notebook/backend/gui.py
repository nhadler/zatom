...
from zatom.tools.ase_notebook.atom_info import create_info_lines
from zatom.tools.ase_notebook.color import lighten_webcolor
from zatom.tools.ase_notebook.draw_utils import initialise_element_groups
...
class AtomGui(GUI):
    ...
    def set_atoms(self, atoms):
        ...
        elements = initialise_element_groups(
            ...
            miller_planes=self.config["miller_planes"] if self.showing_millers() else None,
    ...
    def draw(self, status=True):
        ...
        element_groups.update_positions(
            axes,
            offset,
            radii_scale=self.scale * 0.65 if self.window["toggle-show-bonds"] else self.scale,
        )
        ...
        celldisp = (np.dot(self.atoms.get_celldisp().reshape((3,)), axes)).round().astype(int)
        ...
        element_groups["atoms"].set_property_many(
            {
                ...
                "label": [
                    ...
                    for l, g in zip(self.labels or [None for _ in ghost_atoms], ghost_atoms)
                ],
                ...
                "moving": (
                    [m if self.moving else False for m in self.move_atoms_mask]
                    if self.move_atoms_mask is not None
                    else [False for _ in self.atoms]
                ),
                "occupancy": [
                    self.atoms.info["occupancy"][t] if "occupancy" in self.atoms.info else None
                    for t in self.atoms.get_tags()
                ],
                "stroke_width": [
                    self.config["ghost_stroke_width"] if g else self.config["atom_stroke_width"]
...
def draw_axes(
    ...
    *,
    ...
    for i in axes[:, 2].argsort():
        ...
        canvas.create_line(*tuple(int(x) for x in (a, b, c, d)), width=line_width, fill=line_color)
        canvas.create_text(
            (c, d),
            text="XYZ"[i],
            fill=rgb[i],
...
def draw_elements(
    ...
    for idx, element in element_groups.yield_zorder():
        ...
        if element.name == "miller_planes":
            ...
            plane_pts = [
                pt[i] + celldisp[i] for pt in element.position.round().astype(int) for i in [0, 1]
            ]
