...
def create_svg_document(
    ...
    root.add(shapes.Rect(size=size, fill=background_color, fill_opacity=background_opacity))
...
def concatenate_svgs(
    ...
    svg_composes, dimensions = zip(*[string_to_compose(get_svg_string(svg)) for svg in svgs])
    if scale:
...
def svg_to_pil(svg):
    ...
    buffer = BytesIO()
    cairosvg.svg2png(bytestring=string.encode("utf-8"), write_to=buffer)
    buffer.seek(0)

    # Load the data from the buffer into a PIL Image
