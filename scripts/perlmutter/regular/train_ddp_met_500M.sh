...
#SBATCH --job-name=met-500M                                   # job name
...
DEFAULT_RUN_ID="ehh1zue1"                 # NOTE: Generate a unique ID for each run using `python scripts/generate_id.py`
...
DEFAULT_ARCHITECTURE="met_500M"           # NOTE: Set the model architecture to be used, must be one of (`met_80M`, `met_180M`, `met_500M`, `mft_80M`, `mft_180M`, `mft_500M`)
