...
#SBATCH --time=02-00:00:00                                    # time limit for the job (up to 2 days: `02-00:00:00`)
#SBATCH --job-name=finetune-sweep                             # job name
#SBATCH --output=scripts/perlmutter/regular/logs/finetune_sweep%j.out  # output log file
#SBATCH --error=scripts/perlmutter/regular/logs/finetune_sweep%j.err   # error log file
#SBATCH --array=0-11                                          # create an array of jobs for the sweep (0-11 or 12 total for finetuning and 0-8 or 9 total for generative evaluation)
...
# Define run details
DEFAULT_SWEEP_ID="i8og4qx0"                   # NOTE: Generate a unique ID for each run by running `wandb sweep configs/sweep/{train,eval}_sweep_{joint,}.yaml`
