...
#SBATCH --job-name=tft-70M-joint-ape                          # job name
...
DEFAULT_RUN_ID="f74wcp4r"                 # NOTE: Generate a unique ID for each run using `python scripts/generate_id.py`
DEFAULT_RUN_DATE="2025-12-12_06-00-00"    # NOTE: Set this to the initial date and time of the run for unique identification (e.g., ${now:%Y-%m-%d}_${now:%H-%M-%S})
...
RUN_NAME="${EXPERIMENT}_model-${MODEL}_arch-${ARCHITECTURE}_joint_ape"     # Name of the model type and dataset configuration
