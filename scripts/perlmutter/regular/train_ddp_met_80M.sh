...
#SBATCH --job-name=met-80M                                    # job name
...
DEFAULT_RUN_ID="stsl0lei"                 # NOTE: Generate a unique ID for each run using `python scripts/generate_id.py`
...
DEFAULT_ARCHITECTURE="met_80M"            # NOTE: Set the model architecture to be used, must be one of (`met_80M`, `met_180M`, `met_500M`, `mft_80M`, `mft_180M`, `mft_500M`)
